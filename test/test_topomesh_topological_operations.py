...
from vplants.cellcomplex.property_topomesh.property_topomesh_optimization import (
...
def test_topomesh_edge_flip():
    ...
    for fid in topomesh.wisps(2):
        assert set(topomesh.borders(2, fid, 2)) == set([0, 1, 2]) or set(
            topomesh.borders(2, fid, 2)) == set([1, 2, 3])
    compute_topomesh_property(topomesh, 'area', 2)
    assert np.all(np.isclose(topomesh.wisp_property('area', 2).values(),
                             np.power(side_length, 2.) / 2., 1e-7))

    topomesh_flip_edge(topomesh, 2)

    for fid in topomesh.wisps(2):
        assert set(topomesh.borders(2, fid, 2)) == set([0, 2, 3]) or set(
            topomesh.borders(2, fid, 2)) == set([0, 1, 3])

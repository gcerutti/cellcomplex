...
from tissue.container                     import PropertyTopomesh, array_dict
...
def compute_topomesh_property(topomesh, property_name, degree=0, positions=None, normal_method="density", object_positions=None, object_radius=10., verbose=False):
    ...
    Example:
        >>> from tissue.cellcomplex.property_topomesh.example_topomesh import square_topomesh
        >>> from tissue.cellcomplex.property_topomesh.property_topomesh_analysis import compute_topomesh_property
    ...
    if property_name == 'triangles':
        ...
        if degree < 2:
            topomesh.update_wisp_property('triangles',degree=degree,values=np.array([np.array(list(topomesh.regions(degree,w,2-degree))).astype(int) for w in topomesh.wisps(degree)]),keys=np.array(list(topomesh.wisps(degree))))
        elif degree > 2:
            topomesh.update_wisp_property('triangles',degree=degree,values=np.array([np.array(list(topomesh.borders(degree,w,degree-2))).astype(int) for w in topomesh.wisps(degree)]),keys=np.array(list(topomesh.wisps(degree))))
        else:
            topomesh.update_wisp_property('triangles',degree=degree,values=np.array(list(topomesh.wisps(degree))).astype(int),keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'valence':
        ...
        topomesh.update_wisp_property('valence',degree=degree,values=np.array([len(list(topomesh.region_neighbors(degree,w))) for w in topomesh.wisps(degree)]).astype(int),keys=np.array(list(topomesh.wisps(degree))))
    ...
    if property_name == 'normal':
        if degree == 2:
            ...
            if normal_method == "orientation":
                ...
                from tissue.cellcomplex.property_topomesh.utils.implicit_surfaces import point_spherical_density
    ...
    if property_name in ['principal_curvatures','principal_curvature_tensor','principal_direction_min','principal_direction_max','principal_curvature_min','principal_curvature_max','mean_curvature','gaussian_curvature']:
        if degree == 0:
            ...
            vertex_principal_curvature_min = array_dict([np.sort(v[np.argsort(np.abs(v))[1:]])[0] for v in vertex_curvature_matrix_eigenvalues],epidermis_vertices)
            vertex_principal_curvature_max = array_dict([np.sort(v[np.argsort(np.abs(v))[1:]])[1] for v in vertex_curvature_matrix_eigenvalues],epidermis_vertices)
...
def compute_topomesh_triangle_properties(topomesh,positions=None):
    ...
    Args:
        topomesh (:class:`tissue.cellcomplex.property_topomesh.PropertyTopomesh`):
            The structure on which to compute the property.
        positions (dict, *optional*):
            A position dictionary if the property ('barycenter',0) is empty.
    ...
    Example:
        >>> from tissue.cellcomplex.property_topomesh.example_topomesh import square_topomesh
        >>> from tissue.cellcomplex.property_topomesh.property_topomesh_analysis import compute_topomesh_triangle_properties
        >>> topomesh = square_topomesh(side_length=1)
...
def compute_topomesh_vertex_property_from_faces(topomesh,property_name,weighting='area',neighborhood=1,adjacency_sigma=0.5):
    ...
    Example:
        >>> from tissue.cellcomplex.property_topomesh.example_topomesh import square_topomesh
        >>> from tissue.cellcomplex.property_topomesh.property_topomesh_analysis import compute_topomesh_property
        >>> from tissue.cellcomplex.property_topomesh.property_topomesh_analysis import compute_topomesh_vertex_property_from_faces
        >>> topomesh = square_topomesh(side_length=1)

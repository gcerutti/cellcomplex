...
from vplants.cellcomplex.property_topomesh.utils.matching_tools import kd_tree_match
...
def save_tissue_property_topomesh(topomesh,tissue_filename='vplants.zip'):
    ...
    ptyp = vplants.add_type("point")
    etyp = vplants.add_type("edge")
    ftyp = vplants.add_type("face")
    ctyp = vplants.add_type("cell")

    mesh_id = vplants.add_relation("mesh",(ptyp,etyp,ftyp,ctyp))
    mesh = vplants.relation(mesh_id)

    cells = {}
    for c in topomesh.wisps(3):
        if len(list(topomesh.borders(3,c)))>0:
            cid = vplants.add_element(ctyp)
            mesh.add_wisp(3,cid)
            cells[c] = cid

    faces = {}
    for t in topomesh.wisps(2):
        fid = vplants.add_element(ftyp)
        mesh.add_wisp(2,fid)
        faces[t] = fid

    edges = {}
    for e in topomesh.wisps(1):
        eid = vplants.add_element(etyp)
        mesh.add_wisp(1,eid)
        edges[e] = eid

    points = {}
    for v in topomesh.wisps(0):
        pid = vplants.add_element(ptyp)
        mesh.add_wisp(0,pid)
        points[v] = pid
